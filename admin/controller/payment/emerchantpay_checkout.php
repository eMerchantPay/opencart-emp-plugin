...
class ControllerPaymentEmerchantPayCheckout extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('emerchantpay_checkout', $this->request->post);
		...
		$data = array(
			...
			'emerchantpay_checkout_token'                   => $this->getFieldValue('emerchantpay_checkout_token'),
			...
			'emerchantpay_checkout_async_order_status_id'   => $this->getFieldValue('emerchantpay_checkout_async_order_status_id'),
		...
		$this->response->setOutput($this->load->view('payment/emerchantpay_checkout.tpl', $data));
	}
	...
	public function orderAction() {
		if ($this->config->get('emerchantpay_checkout_status')) {
			...
			$transactions = $this->model_payment_emerchantpay_checkout->getTransactionsByOrder($this->request->get['order_id']);
	...
	public function getModalForm() {
		if (isset($this->request->post['reference_id']) && isset($this->request->post['type'])) {
			...
			$this->response->setOutput($this->load->view('payment/emerchantpay_checkout_order_modal.tpl', $data));
		}
	}
	...
	public function capture() {
		...
		if (isset($this->request->post['reference_id']) && trim($this->request->post['reference_id']) != '') {
			...
			if (isset($transaction['order_id']) && intval($transaction['order_id']) > 0) {
				...
				$capture = $this->model_payment_emerchantpay_checkout->capture($transaction['unique_id'], $amount, $transaction['currency'], $message);

				if (isset($capture->response) && !$capture->error) {
					$amount = $this->model_payment_emerchantpay_checkout->iso4217ConvertAmount($capture->response->amount, $capture->response->currency);
	...
	public function refund() {
		...
		if (isset($this->request->post['reference_id']) && trim($this->request->post['reference_id']) != '') {
			...
			if (isset($transaction['order_id']) && intval($transaction['order_id']) > 0) {
				...
				$refund = $this->model_payment_emerchantpay_checkout->refund($transaction['unique_id'], $amount, $transaction['currency'], $message);

				if (isset($refund->response) && !$refund->error) {
					$amount = $this->model_payment_emerchantpay_checkout->iso4217ConvertAmount($refund->response->amount, $refund->response->currency);
	...
	public function void() {
		...
		if (isset($this->request->post['reference_id']) && trim($this->request->post['reference_id']) != '') {
			...
			if (isset($transaction['order_id']) && intval($transaction['order_id']) > 0) {
				...
				$void = $this->model_payment_emerchantpay_checkout->void($transaction['unique_id'], $message);

				if (isset($void->response) && !$void->error) {
					$amount = $this->model_payment_emerchantpay_checkout->iso4217ConvertAmount($void->response->amount, $void->response->currency);
