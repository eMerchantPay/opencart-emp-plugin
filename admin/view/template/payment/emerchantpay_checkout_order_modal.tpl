...
<div class="modal-dialog" data-type="<?php echo $type;?>">
...
<script type="text/javascript">
    ...
    $(function(){
        ...
        <?php if (($type == 'capture') || ($type == 'refund')) {
            if (($type == 'capture' && !$emerchantpay_checkout_supports_partial_capture) ||
                ($type == 'refund' && !$emerchantpay_checkout_supports_partial_refund)) { ?>
                $('#<?php echo $module_name;?>_transaction_amount').attr('readonly', 'readonly');
            <?php } ?>
        <?php }
        else if (($type == 'void') && (!$transaction['is_allowed'] || !$emerchantpay_checkout_supports_void)) { ?>
